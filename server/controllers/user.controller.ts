...
export const updateAccessToken = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!session) {
        return next(
          new ErrorHandler("Please login ti access this resource!", 400)
        );
      }
      ...
      await redis.set(user._id, JSON.stringify(user), "EX", 604800); // 604800s - 7 days: expired time
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!oldPassword || !newPassword) {
        return next(
          new ErrorHandler("Please enter the requirement values", 400)
        );
      }
